import { SearchCardTarget } from '../../repository/targets/search-card-target.js';
import { Controllers } from '../../controllers/controllers.js';
import { EffectContext } from '../effect-context.js';
import { GameCard } from '../../controllers/card-types.js';

/**
 * Result of search card target resolution.
 */
export type SearchCardTargetResolutionResult =
    | { type: 'resolved', cards: GameCard[] }
    | { type: 'requires-selection', availableCards: GameCard[] }
    | { type: 'no-valid-targets' };

/**
 * Resolves card targets for effects that search or manipulate cards (e.g. SearchEffect).
 * Handles targeting cards in hand, deck, discard pile, or field.
 *
 * Not to be confused with `CardTargetResolver` (`card-target-resolver.ts`), which resolves
 * the `CardTarget` family used by the generic ResolutionRequirement pipeline.
 *
 * @example
 * // Resolve search effect targeting deck
 * SearchCardTargetResolver.resolve(target, controllers, context);
 * // Returns available cards from the deck
 *
 */
export class SearchCardTargetResolver {
    /**
     * Resolves a search card target to available cards.
     *
     * @param target The search card target specification
     * @param controllers Game controllers for accessing card state
     * @param context Effect context
     * @returns A SearchCardTargetResolutionResult indicating available cards or selection requirement
     */
    static resolve(
        target: SearchCardTarget | undefined,
        controllers: Controllers,
        context: EffectContext,
    ): SearchCardTargetResolutionResult {
        if (!target) {
